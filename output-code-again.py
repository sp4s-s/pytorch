...
def pallas_fused_add_mul_relu_sub_e5e40861_kernel(out_ptr0_alias, in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0):
    ...
    x0 = jnp.arange(1024).reshape(32, 32)
    tmp0 = in_ptr0[...]
    tmp1 = in_ptr1[...]
    tmp2 = tmp0 + tmp1
    tmp3 = jnp.maximum(tmp2, 0)
    tmp4 = in_ptr2[...]
    tmp5 = tmp3 * tmp4
    tmp6 = jnp.maximum(tmp5, 0)
    tmp7 = in_ptr3[...]
    tmp8 = tmp6 - tmp7
    tmp9 = jnp.maximum(tmp8, 0)
    out_ptr0[...] = (jnp.full(out_ptr0.shape, tmp9) if jnp.asarray(tmp9).ndim == 0 else (jnp.broadcast_to(jnp.asarray(tmp9), out_ptr0.shape) if jnp.asarray(tmp9).size != out_ptr0.size else jnp.asarray(tmp9).reshape(out_ptr0.shape)))
...
def pallas_fused_add_mul_relu_sub_e5e40861_jit_wrapper(out_shapes, out_dtypes, out_ptr0_alias, in_ptr0, in_ptr1, in_ptr2, in_ptr3):
    ...
    return pl.pallas_call(
        ...
        out_shape=out_specs,
        out_specs=(pl.BlockSpec(out_shapes[0], lambda i: (i, i)),),
        in_specs=[pl.BlockSpec(i.shape, lambda i: (i, i)) for i in [out_ptr0_alias, in_ptr0, in_ptr1, in_ptr2, in_ptr3]],
        interpret=False,
        grid=(1,),
